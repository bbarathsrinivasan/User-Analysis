"""
Subset pipeline script for processing trade data - excludes incomplete events.
This script only processes complete markets, excluding:
...
# Events to exclude (incomplete data)
EXCLUDED_EVENTS = {
    "arizona-senate-election-margin-of-victory",
    "next-senate-majority-leader"
}
...
def process_market(event_id, market_slug, base_path="raw", output_base_path="subset_all_markets_output"):
    """
    ...
    Args:
        ...
        output_base_path: Base path for output directory (default: "subset_all_markets_output")
...
def main():
    ...
    Excludes events with incomplete data.
    """
    base_path = "raw"
    output_base_path = "subset_all_markets_output"
    ...
    event_dirs = [d for d in event_dirs if d.name not in EXCLUDED_EVENTS]
    
    if not event_dirs:
        print(f"Warning: No event directories found in {events_path} (after excluding incomplete events)")
        return
    
    excluded_count = len([d for d in events_path.iterdir() if d.is_dir() and not d.name.startswith('.')]) - len(event_dirs)
    print(f"Found {len(event_dirs)} event(s) to process (excluded {excluded_count} incomplete event(s))")
    ...
    for event_dir in sorted(event_dirs):
        ...
        for trade_file in sorted(trade_files):
            ...
            try:
                final_df = process_market(event_id, market_slug, base_path, output_base_path)
    ...
    if all_markets_data:
        ...
        try:
            ...
            # Write aggregated output files for each user and token type (using subset output path)
            for _, row in user_token_combinations.iterrows():
                user_id = row['user_id']
                token_type = row['token_type']
                write_all_markets_user_output(final_aggregated_df, user_id, token_type, base_path=output_base_path)
            
            # Write combined token files for each user (using subset output path)
            for user_id in unique_users:
                write_all_markets_combined_token_output(final_aggregated_df, user_id, base_path=output_base_path)
            
            # Write date_group_token.csv files for each user (using subset output path)
            for user_id in unique_users:
                write_date_group_token_output(user_id, base_path=output_base_path)
