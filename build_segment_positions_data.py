...
For each event in data/<event_id>/trades/*.csv and each market_slug,
this script writes:
...
RAW_BASE = Path("data")
OUTPUT_BASE = Path("data_segment_output")
...
def load_daily_closing_prices(event_id: str, market_slug: str) -> Optional[pd.DataFrame]:
    ...
    Assumes a file:
      data/<event_id>/prices/<market_slug>_closing_prices.csv
      or data/<event_id>/prices/<market_slug>_price.csv
    ...
    if "date" in df.columns and "token_type" in df.columns:
        ...
        # Get closing price per day (last price of each day by timestamp)
        df = df.sort_values('timestamp')
        daily_closing = df.groupby(['date', 'token_type']).last().reset_index()
        
        # Pivot to get yes_token_price and no_token_price
...
def main():
    print("Building per-market, per-user segment output from data folder trades...\n")

    if not RAW_BASE.exists():
        print(f"Data base directory not found: {RAW_BASE}")
        return
